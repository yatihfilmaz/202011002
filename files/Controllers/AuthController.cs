    ...
    public class AuthController : Controller
    {
        ...
        public async Task<IActionResult> ForgotPassword(string email)
        {
            ...
            if (user == null)
            {
                ...
                TempData["Success"] =
                    "Eğer sistemimizde bu e-posta adresiyle kayıtlı bir hesabınız varsa, şifre sıfırlama bağlantısı gönderilmiştir.";
        ...
        public async Task<IActionResult> ResetPassword(string token)
        {
            ...
            // Token veritabanında var mı ve süresi dolmuş mu kontrolü
            var user = await _context.Users.FirstOrDefaultAsync(u => u.ResetPasswordToken == token);

            if (user == null || user.ResetTokenExpires < DateTime.Now)
            {
                TempData["Error"] =
                    "Şifre sıfırlama bağlantısı geçersiz veya süresi dolmuş. Lütfen tekrar talepte bulunun.";
                return RedirectToAction("ForgotPassword");
            }

            // Her şey yolundaysa ViewModel'i token ile birlikte View'a gönderiyoruz
        ...
        public async Task<IActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            // --- MUTLU SON: Yeni şifreyi BCrypt ile şifrele ve kaydet ---
            ...
            // --- LOGLAMA (Opsiyonel ama güzel olur) ---
            var log = new SystemLog
            {
                ActionType = "Şifre Sıfırlama",
                ...
                if (user != null && BCrypt.Net.BCrypt.Verify(model.Password, user.PasswordHash))
                {
                    if (!user.IsEmailVerified)
                    {
                        ModelState.AddModelError("",
                            "Giriş yapabilmek için lütfen önce e-posta adresinize gönderilen linkten hesabınızı doğrulayın.");
                        return View(model);
                    }

                    if (user.RoleId == 3 && !user.IsApproved)
                    {
                        ModelState.AddModelError(string.Empty,
                            "Tedarikçi başvurunuz yönetici onayındadır. Onaylandıktan sonra giriş yapabilirsiniz.");
                        return View(model);
                    }
                    ...
                    var claims = new List<Claim>
                    {
                        ...
                        // Rolünü veritabanından aldığımız RoleID'ye göre atıyoruz (Admin=1, Caterer=2, User=3 varsayımıyla)
                        new Claim(ClaimTypes.Role,
                            user.RoleId == 1 ? "Admin" : (user.RoleId == 2 ? "Caterer" : "Customer")),
                    };
                    ...
                    // .NET Core'a "Bu kullanıcıyı sisteme giriş yapmış olarak işaretle" diyoruz
                    await HttpContext.SignInAsync(
                        CookieAuthenticationDefaults.AuthenticationScheme,
                        new ClaimsPrincipal(claimsIdentity),
                        authProperties);

                    // Giriş başarılı, rolüne göre Dashboard'a yönlendir
                    if (user.RoleId == 1) return RedirectToAction("Index", "Admin");
                    if (user.RoleId == 2) return RedirectToAction("Index", "Caterer");

                    var log = new SystemLog
                    {
                        ActionType = "Giriş Başarılı",
                    ...
                    return RedirectToAction("Index", "Home"); // Normal kullanıcı
                }

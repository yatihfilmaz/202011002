...
public partial class DenemeContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        {
            // Birincil anahtarı belirtiyoruz
            entity.HasKey(e => e.RatingId);

            // Tablo adını tam olarak eşleştiriyoruz
            entity.ToTable("ItemRatings");

            // Bir yemeğin (MenuItem) birden fazla puanı (ItemRatings) olabilir ilişkisi
            entity.HasOne(d => d.Item)
                .WithMany(p => p.ItemRatings)
                .HasForeignKey(d => d.ItemId)
                .OnDelete(DeleteBehavior.Cascade) // Yemek silinirse puanları da silinsin
                .HasConstraintName("ItemRatings_MenuItems_ItemId_fk");
        ...
        modelBuilder.Entity<MenuComboItem>()
            .HasKey(mci => new { mci.ComboId, mci.SubItemId });

        modelBuilder.Entity<MenuComboItem>()
            .HasOne(mci => mci.Combo)
            .WithMany(m => m.ComboContents)
            .HasForeignKey(mci => mci.ComboId)
            .OnDelete(DeleteBehavior.Cascade);

        modelBuilder.Entity<MenuComboItem>()
            .HasOne(mci => mci.SubItem)
            .WithMany(m => m.IncludedInCombos)
            .HasForeignKey(mci => mci.SubItemId)
